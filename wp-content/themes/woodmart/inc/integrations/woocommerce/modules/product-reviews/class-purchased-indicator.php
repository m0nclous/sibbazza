...
class Purchased_Indicator {
	...
	public function add_author_icon( $author, $comment_ID ) {
		if ( ( $this->is_parent_comment( $comment_ID ) && ( ! $this->is_user_purchased_product( $comment_ID ) || ! woodmart_get_opt( 'show_reviews_purchased_indicator' ) ) ) || ( ! wp_doing_ajax() && is_admin() ) ) {
			return $author;
		}

		$tooltip_text = $this->is_parent_comment( $comment_ID ) ? 'Verified owner' : 'Store manager';
		...
		<span class="wd-review-icon">
			<span class="wd-tooltip">
				<?php echo esc_html__( $tooltip_text, 'woodmart' ); ?>
			</span>
		</span>
		<?php echo $author; ?>
		<?php
